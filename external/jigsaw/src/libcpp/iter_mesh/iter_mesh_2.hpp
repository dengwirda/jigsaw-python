...
#       endif//__use_timers
                    ...
                    multi_nset.set_count(num_threads, containers::tight_alloc);
                    ...
                    if (num_threads > 1) {
                        ...
                        auto reduction_task = [&](auto r, auto c) {
                            if (multi_nset[r + (c / 2)].count() > +0)
                                for (auto it = multi_nset[r + (c / 2)].head();
                                     it != multi_nset[r + (c / 2)].tend(); ++it)
