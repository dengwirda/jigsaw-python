...
#--------------------------- compute signed area-len. ratios
    ...
    lrms = np.sum((pos2-pos1)**2, axis=1) \
...
#----------------------------------- compute enclosed angles
    ...
    dcos[:,0] = np.sum(-ee11*ee22, axis=1)
    dcos[:,1] = np.sum(-ee22*ee33, axis=1)
    dcos[:,2] = np.sum(-ee33*ee11, axis=1)
    
    dcos[:,0] = np.maximum(-1., dcos[:,0])
    dcos[:,0] = np.minimum(+1., dcos[:,0])
    dcos[:,1] = np.maximum(-1., dcos[:,1])
    dcos[:,1] = np.minimum(+1., dcos[:,1])
    dcos[:,2] = np.maximum(-1., dcos[:,2])
    dcos[:,2] = np.minimum(+1., dcos[:,2])
...
#----------------------------------- compute enclosed angles
    ...
    ff11 = normal2(ppos, tri3[:,(0,1,2)])
    ff22 = normal2(ppos, tri3[:,(0,1,3)])
    ff33 = normal2(ppos, tri3[:,(1,2,3)])
    ff44 = normal2(ppos, tri3[:,(2,0,3)])
    ...
    dcos[:,0] = np.sum(+ff11*ff22, axis=1)
    dcos[:,1] = np.sum(+ff11*ff33, axis=1)
    dcos[:,2] = np.sum(+ff11*ff44, axis=1)
    dcos[:,3] = np.sum(-ff44*ff22, axis=1)
    dcos[:,4] = np.sum(-ff22*ff33, axis=1)
    dcos[:,5] = np.sum(-ff33*ff44, axis=1)
    ...
    dcos[:,2] = np.maximum(-1., dcos[:,2])
    dcos[:,2] = np.minimum(+1., dcos[:,2])
    dcos[:,3] = np.maximum(-1., dcos[:,3])
    dcos[:,3] = np.minimum(+1., dcos[:,3])
    dcos[:,4] = np.maximum(-1., dcos[:,4])
    dcos[:,4] = np.minimum(+1., dcos[:,4])
    dcos[:,5] = np.maximum(-1., dcos[:,5])
    dcos[:,5] = np.minimum(+1., dcos[:,5])
